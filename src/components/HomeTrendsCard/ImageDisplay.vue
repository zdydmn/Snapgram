    ...
    <div>
        <div class="w-imgCardW h-imgCardH overflow-hidden relative rounded-3xl">
            <div class="flex w-max transition-all w-full h-full items-center"
                ...
                <!-- 图片变换 -->
                <!-- <div v-for="(item, index) in  img_list " :id="item" :key="index" :style="{
                    background: index === 0 ? 'red' :
                        (index === 1 ? 'yellow' : 'green')
                }" class="w-full h-full flex-auto">

                </div> -->
                <img v-for="(item, index) in img_list" :id="item" :key="index" :src="item"
                    class="w-imgCardW h-imgCardH grow-0" />
            </div>
            <div class="absolute gap-x-4 flex bottom-2 right-1/2 translate-x-2/4" v-if="img_list.length > 1">
                <div class="w-2.5 h-2.5 bg-light4 rounded-full hover:bg-white transition-all hover:cursor-pointer"
                    :class="[isActive === ind ? 'bg-white' : '']" v-for="( i, ind ) in  img_list " :key="i"
                    @click="jump(ind)">
                </div>
            </div>
            <div class="absolute bottom-1/2 right-2" v-if="img_list.length > 1">
                <button @click="fn"
            ...
            <div class="absolute bottom-1/2 left-2" v-if="img_list.length > 1">
                <button @click="fn2"
                    class=" w-7 h-7 rounded-full bg-imgButton flex items-center justify-center hover-img-button">
...
const fn = () => {
    if (!img_list) return 
    if (isActive.value === img_list.length - 1) {
        return
    }
